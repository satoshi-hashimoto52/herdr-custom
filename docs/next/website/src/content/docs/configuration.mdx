...
- `state_icon` — colored icon for the agent's semantic state. On dark themes the state colors keep their palette hue and are lifted toward the fluorescent end of it, so the state reads at a glance without recoloring the rest of the sidebar.
- `state_text` — `idle`, `working`, `waiting`, `done`, or `error`, including a reported display label when present. `waiting` is the detected `blocked` state named for what you are waiting on; `error` means the agent process exited mid-turn.

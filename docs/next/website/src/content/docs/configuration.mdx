...
- `state_icon` — colored icon for the agent's semantic state, in the same color as the state word beside it. `run` takes the theme's accent blue, a color that sits with the rest of the interface rather than shouting over it; `wait` takes its mauve, since being asked a question is the agent working normally, not a fault; and `err` keeps the red, which now belongs to it alone, so a row waiting for you and a row that failed are told apart by color and not only by the word. `idle`, `done`, and `unknown` keep their palette hue, lifted toward the fluorescent end of it on dark themes so the state reads at a glance without recoloring the rest of the sidebar. A theme that ships the same color for both its accent and its mauve, and one built from ANSI color names, keep the earlier yellow and red for these two states instead. This is display color only — the state machine, the internal state names, and the socket API are unchanged.
- `state_text` — `idle`, `run`, `wait`, `done`, or `err`, including a reported display label when present. The words are short, four characters at most, because a row spends its width on the agent name and the working time and the state only has to be recognised; the color and the marker carry the same distinction. `run` means the agent is processing, `wait` is the detected `blocked` state named for what you are waiting on — your input or instruction — and `err` means the agent process exited mid-turn or otherwise needs attention. These are displayed labels only: the internal state names, the detector's own names, and the socket API keep `idle`/`working`/`blocked`/`done`/`unknown`, and a `state_labels` override is still keyed by the long name.
- `workspace` — workspace name.
- `tab` — tab name when available. It renders in a warm accent, whatever the tab is called, so the project a row belongs to can be picked out of the list; the workspace name, the separator, the agent name, and the state keep their own colors.
...
### Sidebar resource footer

The expanded desktop sidebar pins free disk space and swap use to its bottom rows, below the Agents list, so the Agents list scrolls while the readings stay visible:
...
Readings come from OS APIs rather than shelled-out commands: `statfs` on the APFS Data volume plus `vm.swapusage` on macOS, and `statvfs` with `/proc/meminfo` on Linux. Free space is `f_bavail` — the space an unprivileged process can actually claim, the same figure `df` reports. It need not match what Finder and System Settings show, because those add reclaimable purgeable space to their total. Values are in decimal gigabytes.

Free space is read once at startup and every 30 seconds after that. Swap keeps its own four-second beat. Either way sampling runs only while the expanded sidebar is on screen — a collapsed sidebar, the mobile layout, and `enabled = false` all stop it. A reading holds still when it has not changed, so the footer does not flicker.

The free-space row marks its own refreshes:

```text
SSD   198 GB ✓   just re-read
SSD   198 GB !   the last read failed
SSD   198 GB     steady state
```

The green `✓` stays up for five seconds after a successful read, including a read that returns the same number, and a second success inside those five seconds restarts them. A failed read keeps the last good value and shows a yellow `!` until a read succeeds, at which point the `!` becomes a fresh `✓`. Before the first successful read there is no value to qualify, so the row stays hidden rather than showing a bare marker.

Free space reads muted above 50 GB, yellow from 30 to 50 GB, peach from 20 to 30 GB, and bold red below 20 GB. Swap reads muted below 5 GB, yellow from 5 to 10 GB, and peach above that. Swap stays visible at `0.0 GB`. A narrow sidebar abbreviates the readings to `198G` and `0.0G`, and drops the refresh marker before it would cost the number a digit; a sidebar too short to spare three rows hides the footer and keeps its sections. Platforms without a supported query hide the footer instead of reporting a zero.

### Agent working time

Each Agent row prints how long its agent has actually spent working, next to
the state word:
...
This is measured time, not an estimate of progress: there is no percentage and
no bar, because nothing here can know how much of a task is left. What it can
know is how much work has gone in.

| State | Figure |
| --- | --- |
| `idle` | none — the next turn starts again from zero |
| `run` | counting up, once a second |
| `wait` | held at the total so far; waiting on a person is not working |
| `done` | the final total, frozen |
| `err` | the total as of the failure, frozen |

Answering a question resumes the same turn from where it paused. Starting new
work from `done` or `err` begins again at `00:00`. A turn that opens by
asking something has done no work yet, so nothing is printed beside `wait`
until work actually starts, and the previous turn's total is never carried into
the next one.

Working time is measured from the state changes themselves rather than sampled
while drawing, so it survives everything short of the process ending: reloading
your configuration, collapsing the sidebar, selecting a different agent, and a
repeated report of the state a pane is already in all leave a running total
alone. It is held in memory for the life of the server process, though —
restarting the server, and a live handoff, which is a restart that keeps your
panes, start every row from zero again. Nothing is written to disk and no
figure is carried across.

There is nothing to configure. The figure adds no settings of its own, and it
costs no polling: the row is redrawn on the second the number changes and at no
other time, on the loop Herdr already runs.

Under an hour the figure reads `MM:SS`, at or over one `H:MM:SS`, and hours are
not wrapped at 24. Part-seconds are dropped rather than rounded up, so it never
reads ahead of the work. It is drawn in the sidebar's muted secondary color
rather than the state's own, so the state word keeps carrying the state.

The figure is the lowest-priority thing on the row: a sidebar too narrow to
hold it drops it whole rather than cutting it to `01:`, and never trades the
agent name or the state word for it. Rows keep the two-row shape they always
had, and the row only redraws on the second it actually changes — a list of
finished agents costs nothing at all.

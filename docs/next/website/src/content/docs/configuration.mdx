...
Working time is held in memory for the life of the server process. Restarting
the server — and a live handoff, which is a restart that keeps your panes —
starts every row from zero again. Nothing is written to disk and no figure is
carried across.

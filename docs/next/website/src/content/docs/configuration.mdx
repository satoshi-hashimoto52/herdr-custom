...
- `state_text` — `idle`, `running`, `waiting`, `done`, or `error`, including a reported display label when present. `running` and `waiting` are the detected `working` and `blocked` states named for what you are waiting on; `error` means the agent process exited mid-turn.

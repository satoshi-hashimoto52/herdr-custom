...
Readings come from OS APIs rather than shelled-out commands: `statfs` on the APFS Data volume plus `vm.swapusage` on macOS, and `statvfs` with `/proc/meminfo` on Linux. Disk space is the space a process can actually claim, which tracks `df` and `diskutil` rather than the larger purgeable-inclusive figure Finder and System Settings show. Values are in decimal gigabytes.

Sampling runs every four seconds, and only while the expanded sidebar is on screen — a collapsed sidebar, the mobile layout, and `enabled = false` all stop it. A reading holds still when it has not changed, so the footer does not flicker.

Free space reads muted above 50 GB, yellow from 30 to 50 GB, peach from 20 to 30 GB, and bold red below 20 GB. Swap reads muted below 5 GB, yellow from 5 to 10 GB, and peach above that. Swap stays visible at `0.0 GB`. A narrow sidebar abbreviates the readings to `198G` and `0.0G`; a sidebar too short to spare three rows hides the footer and keeps its sections. Platforms without a supported query hide the footer instead of reporting a zero.

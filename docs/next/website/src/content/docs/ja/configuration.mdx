...
- `state_text` — `idle`、`running`、`waiting`、`done`、`error`。報告された表示ラベルがある場合はそれも含みます。`running` と `waiting` は、検出された `working` と `blocked` を「何を待っているか」で言い換えたものです。`error` はエージェントのプロセスが処理中に終了したことを示します。
- `workspace` — ワークスペース名。
- `tab` — 利用可能な場合はタブ名。
- `pane` — 利用可能な場合はペイン名。

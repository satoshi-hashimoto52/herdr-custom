...
- `state_icon` — エージェントの意味的な状態を示す色付きアイコン。暗いテーマでは、パレット固有の色相を保ったまま蛍光寄りの彩度・明度へ補正され、サイドバーの他の文字を明るくすることなく状態がひと目で分かります。
- `state_text` — `idle`、`working`、`waiting`、`done`、`error`。報告された表示ラベルがある場合はそれも含みます。`waiting` は、検出された `blocked` を「何を待っているか」で言い換えたものです。`error` はエージェントのプロセスが処理中に終了したことを示します。
- `workspace` — ワークスペース名。

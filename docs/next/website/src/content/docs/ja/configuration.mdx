...
値はシェルコマンドを起動せず、OS API から直接取得します。macOS では APFS Data ボリュームに対する `statfs` と `vm.swapusage`、Linux では `statvfs` と `/proc/meminfo` を使います。空き容量はプロセスが実際に確保できる容量で、Finder やシステム設定が表示する purgeable を含む大きい値ではなく、`df` や `diskutil` と一致します。単位は 10 進ギガバイトです。

取得は 4 秒間隔で、展開されたサイドバーが表示されているときだけ実行されます。サイドバーを折りたたんだ場合、モバイルレイアウトの場合、`enabled = false` の場合はいずれも停止します。値が変わらないときは再描画しないため、フッターがちらつくことはありません。

空き容量は 50 GB 超で控えめな色、30〜50 GB で黄、20〜30 GB でピーチ、20 GB 未満で太字の赤になります。Swap は 5 GB 未満で控えめな色、5〜10 GB で黄、それ以上でピーチになります。Swap が 0 の場合も `0.0 GB` として表示は消えません。サイドバーが狭いときは `198G`、`0.0G` のように省略し、3 行を確保できないほど低いときはフッターを隠して各セクションを維持します。対応する取得手段がないプラットフォームでは、0 を表示するのではなくフッター自体を隠します。

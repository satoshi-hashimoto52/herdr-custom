...
- `state_icon` — エージェントの意味的な状態を示す色付きアイコン。隣の状態文字と常に同じ色です。`run` はテーマのアクセントブルーで、主張しすぎず UI へ馴染みます。`wait` はモーブ（パープル）で、質問を返してくるのは異常ではなく正常な指示待ちだからです。`err` は赤のままで、いまやこの状態だけが赤を使うため、応答を待っている行と失敗した行を言葉だけでなく色でも見分けられます。`idle`、`done`、`unknown` はパレット固有の色相を保ったまま、暗いテーマでは蛍光寄りの彩度・明度へ補正され、サイドバーの他の文字を明るくすることなく状態がひと目で分かります。アクセントとモーブに同じ色を持つテーマや、ANSI カラー名で組まれたテーマでは、この 2 状態は従来どおりの黄と赤になります。変更されるのは表示色だけで、状態機械・内部状態名・socket API は従来どおりです。
- `state_text` — `idle`、`run`、`wait`、`done`、`err`。報告された表示ラベルがある場合はそれも含みます。行の幅はエージェント名と作業時間に使いたく、状態は読むより見分けられれば足りるため、最長 4 文字の短縮表記です。同じ区別を色とアイコンも担うので判別性は落ちません。`run` は処理中、`wait` は検出された `blocked` を「何を待っているか」で言い換えたもの（ユーザーの入力・指示待ち）、`err` はエージェントのプロセスが処理中に終了した、または対応が必要な状態です。変わるのは表示文字列だけで、内部状態名・検出器の名前・socket API は `idle` / `working` / `blocked` / `done` / `unknown` のまま、`state_labels` の上書きキーも長い名前のままです。
- `workspace` — ワークスペース名。
- `tab` — 利用可能な場合はタブ名。タブ名が何であっても暖色のアクセントで描画され、その行がどのプロジェクトのものか一目で分かります。ワークスペース名・区切り・エージェント名・状態表示は従来の色のままです。
- `pane` — 利用可能な場合はペイン名。
...
展開されたデスクトップサイドバーでは、SSD の空き容量と Swap 使用量が Agents 一覧の下、最下部に固定表示されます。Agents 一覧をスクロールしても、この表示は見えたままです:
...
値はシェルコマンドを起動せず、OS API から直接取得します。macOS では APFS Data ボリュームに対する `statfs` と `vm.swapusage`、Linux では `statvfs` と `/proc/meminfo` を使います。空き容量は `f_bavail`、つまり非特権プロセスが実際に確保できる容量で、`df` が報告する値と同じです。Finder やシステム設定は解放可能な purgeable 領域を加えて表示するため、これらの数値と一致するとは限りません。単位は 10 進ギガバイトです。

空き容量は起動時に 1 回取得し、以降は 30 秒ごとに取得します。Swap は従来どおり 4 秒間隔です。いずれも展開されたサイドバーが表示されているときだけ実行され、サイドバーを折りたたんだ場合、モバイルレイアウトの場合、`enabled = false` の場合はいずれも停止します。値が変わらないときは再描画しないため、フッターがちらつくことはありません。

空き容量の行には、最後にいつ取得できたかを示すマーカーが付きます:
...
緑の `✓` は取得成功から 5 秒間表示されます。値が変わらなかった場合でも、正常に再取得できていれば表示されます。5 秒以内にもう一度成功した場合は、その時点から 5 秒を数え直します。取得に失敗したときは最後に取得できた値をそのまま保持し、次に成功するまで黄色の `!` を表示します。成功した時点で `!` は消え、あらためて緑の `✓` が 5 秒間表示されます。一度も取得に成功していないうちは対象となる値がないため、マーカーだけを出すのではなく行ごと非表示のままです。

空き容量は 50 GB 超で控えめな色、30〜50 GB で黄、20〜30 GB でピーチ、20 GB 未満で太字の赤になります。Swap は 5 GB 未満で控えめな色、5〜10 GB で黄、それ以上でピーチになります。Swap が 0 の場合も `0.0 GB` として表示は消えません。サイドバーが狭いときは `198G`、`0.0G` のように省略し、数値を削るより先にマーカーを省きます。3 行を確保できないほど低いときはフッターを隠して各セクションを維持します。対応する取得手段がないプラットフォームでは、0 を表示するのではなくフッター自体を隠します。

### Agent の作業時間

各 Agent 行には、そのエージェントが実際に作業していた時間が状態文字の右に表示されます:
...
これは実測時間であって進捗の推定ではありません。残作業量は知りようがないため、
パーセント表示もプログレスバーもありません。分かるのは投じられた作業量だけです。

| 状態 | 表示 |
| --- | --- |
| `idle` | 非表示。次のターンはまた 0 から始まる |
| `run` | 1 秒ごとに加算 |
| `wait` | それまでの累積で停止。人の応答待ちは作業ではない |
| `done` | 最終累積時間で固定 |
| `err` | エラー時点の累積時間で固定 |

質問に答えると、停止した時点から同じターンが再開します。`done` や `err` から
新しい作業を始めた場合は `00:00` からのやり直しです。作業を始める前に質問から入った
ターンにはまだ作業時間がないため、実際に作業が始まるまで `wait` の横には何も表示
されません。前のターンの累計が次のターンへ持ち越されることもありません。

作業時間は、描画時に状態を読み取るのではなく状態遷移そのものから計測しています。
そのためプロセスが終わらない限り値は保たれ、設定の再読み込み、サイドバーの折りたたみ、
別のエージェントの選択、同じ状態の重複報告のいずれでも、計測中の値はリセットされま
せん。ただし保持はサーバープロセスが生きている間だけです。サーバーの再起動（および
ペインを保ったまま再起動する live handoff）では、すべての行が 0 から始まります。
ディスクへの保存は行わず、値の引き継ぎもありません。

設定項目はありません。この表示のために増えた設定キーはなく、ポーリングも行いません。
再描画は数値が変わるその 1 秒だけで、しかも Herdr が元から回しているループの上で
行われます。

1 時間未満は `MM:SS`、1 時間以上は `H:MM:SS` で、24 時間で折り返したりはしません。
秒未満は切り上げずに切り捨てるので、実際の作業より先に進んで見えることはありません。
色は状態色ではなくサイドバーの控えめな二次色で、状態を伝える役目は状態文字が持ち
続けます。

この時間は行の中で最も優先度が低い要素です。サイドバーが狭くて収まらないときは
`01:` のように切るのではなく丸ごと省略し、エージェント名や状態文字を犠牲にすることは
ありません。行は従来どおり 2 行のままで、再描画は表示が実際に変わる秒だけ行われます。
完了済みのエージェントが並んでいるだけなら再描画コストはゼロです。

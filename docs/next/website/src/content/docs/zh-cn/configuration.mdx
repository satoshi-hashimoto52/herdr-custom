...
读数直接来自操作系统 API，而不是调用外部命令：macOS 使用针对 APFS Data 卷的 `statfs` 和 `vm.swapusage`，Linux 使用 `statvfs` 与 `/proc/meminfo`。磁盘空间是进程实际可以占用的空间，与 `df` 和 `diskutil` 一致，而不是 Finder 和系统设置显示的、包含可清除空间的更大数值。单位为十进制 GB。

采样每四秒一次，且仅在展开的侧边栏可见时进行；折叠侧边栏、移动布局以及 `enabled = false` 都会停止采样。读数未变化时不会重绘，因此页脚不会闪烁。

可用空间在 50 GB 以上显示为柔和色，30 至 50 GB 为黄色，20 至 30 GB 为桃色，低于 20 GB 为加粗红色。交换区在 5 GB 以下为柔和色，5 至 10 GB 为黄色，更高为桃色。交换区为 0 时仍显示 `0.0 GB`。侧边栏较窄时会缩写为 `198G` 和 `0.0G`；高度不足三行时会隐藏页脚并保留各分区。无法获取这些数据的平台会隐藏页脚，而不是显示 0。

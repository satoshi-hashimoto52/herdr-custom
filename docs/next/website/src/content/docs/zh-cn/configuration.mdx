...
- `state_icon` — 智能体语义状态的彩色图标，颜色始终与旁边的状态词一致。`run` 使用主题的强调蓝，融入界面而不喧宾夺主；`wait` 使用淡紫（mauve），因为智能体提出问题属于正常的等待指示，而非故障；`err` 保持红色，如今红色只属于它，因此“在等你回答的行”和“出错的行”不仅靠文字、也能靠颜色区分。`idle`、`done`、`unknown` 保留调色板自身的色相，在深色主题下向荧光方向提升饱和度与明度，无需调亮侧边栏其他文字即可一眼看出状态。若某个主题的 accent 与 mauve 是同一种颜色，或主题由 ANSI 颜色名构成，这两个状态将沿用原先的黄色与红色。这里改变的只有显示颜色，状态机、内部状态名与 socket API 均保持不变。
- `state_text` — `idle`、`run`、`wait`、`done` 或 `err`；如果报告中包含显示标签，也会一并显示。行宽要留给智能体名称和工作时长，状态只需被认出而不必被读完，因此这些词最长四个字符；颜色与图标承担同样的区分作用，缩短后并不会更难辨认。`run` 表示正在处理，`wait` 是检测到的 `blocked` 状态，按“你在等什么”重新命名（等待用户输入或指示），`err` 表示智能体进程在处理过程中退出或需要处理。这里改变的只是显示文字：内部状态名、检测器自身的名称与 socket API 仍为 `idle` / `working` / `blocked` / `done` / `unknown`，`state_labels` 覆盖也仍以长名称为键。
- `workspace` — 工作区名称。
- `tab` — 标签页名称（如有）。无论标签页叫什么，都以暖色强调渲染，便于一眼看出该行属于哪个项目；工作区名称、分隔符、智能体名称与状态显示保持原有颜色。
- `pane` — 窗格名称（如有）。
...
展开的桌面侧边栏会把可用磁盘空间和交换区用量固定在底部、Agents 列表下方，因此 Agents 列表滚动时这些读数仍然可见：
...
读数直接来自操作系统 API，而不是调用外部命令：macOS 使用针对 APFS Data 卷的 `statfs` 和 `vm.swapusage`，Linux 使用 `statvfs` 与 `/proc/meminfo`。可用空间取 `f_bavail`，即非特权进程实际可以占用的空间，与 `df` 报告的数值相同。Finder 和系统设置会把可清除空间计入总量，因此二者不一定一致。单位为十进制 GB。

可用空间在启动时读取一次，此后每 30 秒读取一次；交换区仍保持四秒一次。两者都只在展开的侧边栏可见时采样；折叠侧边栏、移动布局以及 `enabled = false` 都会停止采样。读数未变化时不会重绘，因此页脚不会闪烁。

可用空间这一行会标记自己最近一次刷新的结果：
...
读取成功后，绿色的 `✓` 会显示五秒；即使数值没有变化，只要重新读取成功就会显示。若在这五秒内再次成功，则从该时刻重新计时五秒。读取失败时保留最后一次成功的数值，并显示黄色的 `!`，直到某次读取成功；成功后 `!` 消失，重新显示五秒的绿色 `✓`。在第一次成功读取之前没有可标注的数值，因此该行保持隐藏，而不是只显示一个标记。

可用空间在 50 GB 以上显示为柔和色，30 至 50 GB 为黄色，20 至 30 GB 为桃色，低于 20 GB 为加粗红色。交换区在 5 GB 以下为柔和色，5 至 10 GB 为黄色，更高为桃色。交换区为 0 时仍显示 `0.0 GB`。侧边栏较窄时会缩写为 `198G` 和 `0.0G`，并且会先省略刷新标记，再考虑截断数值；高度不足三行时会隐藏页脚并保留各分区。无法获取这些数据的平台会隐藏页脚，而不是显示 0。

### Agent 工作时长

每个 Agent 行都会在状态词旁边显示该智能体实际工作了多久：
...
这是实测时间，而不是进度估算：没有百分比，也没有进度条，因为这里无从知道任务还剩多少。
能知道的只是已经投入了多少工作。
...
| `idle` | 不显示；下一轮从零重新开始 |
| `run` | 每秒递增 |
| `wait` | 停在当前累计值；等待用户答复不算工作 |
| `done` | 固定为最终累计时长 |
| `err` | 固定为出错时刻的累计时长 |

回答问题后会从暂停处继续同一轮计时。从 `done` 或 `err` 开始新的工作则从 `00:00`
重新计时。若某一轮一开始就提问，此时还没有任何工作时间，因此在真正开始工作之前
`wait` 旁不会显示任何数值，上一轮的累计也绝不会被带入下一轮。

工作时长由状态变化本身计量，而不是在绘制时采样，因此只要进程还在，数值就会保留：
重新加载配置、收起侧边栏、切换选中的智能体，以及对同一状态的重复上报，都不会重置
正在计时的数值。不过它只保存在服务端进程的内存中——重启服务端（以及 live handoff，
即保留窗格的重启）之后，所有行都会从零重新开始。这些数值不会写入磁盘，也不会被继承。

没有任何配置项。这项显示不会新增配置键，也不进行轮询：只在数字真正变化的那一秒重绘，
而且用的是 Herdr 本来就在运行的循环。

不足一小时显示为 `MM:SS`，达到或超过一小时显示为 `H:MM:SS`，且不会在 24 小时处回绕。
不足一秒的部分向下取整而非向上，因此显示不会超前于实际工作。它使用侧边栏柔和的次级
颜色而不是状态色，传达状态的职责仍由状态词承担。

这个时长是行内优先级最低的元素：侧边栏窄到放不下时会整个省略，而不会截断成 `01:`，
更不会为它牺牲智能体名称或状态词。行仍保持一贯的两行，并且只在显示真正变化的那一秒
重绘——一列已完成的智能体不会带来任何重绘开销。

...
- `state_icon` — 智能体语义状态的彩色图标。在深色主题下保留调色板自身的色相，并向荧光方向提升饱和度与明度，无需调亮侧边栏其他文字即可一眼看出状态。
- `state_text` — `idle`、`working`、`waiting`、`done` 或 `error`；如果报告中包含显示标签，也会一并显示。`waiting` 是检测到的 `blocked` 状态，按“你在等什么”重新命名；`error` 表示智能体进程在处理过程中退出。
- `workspace` — 工作区名称。

# Herdr fork への切り替えメモ（2026-09-04）

## 何が変わったか

Ghostty が起動する Herdr を、Homebrew 版から自前ビルド版へ切り替えました。

| | パス | 版 |
|---|---|---|
| 切り替え前 | `/opt/homebrew/bin/herdr` | 0.8.0 (Homebrew) |
| 切り替え後 | `~/.local/bin/herdr` | 0.8.2 + 今回の改修 |

変更したのは `~/.config/ghostty/config` の 14 行目 `initial-command` のみです。

- バックアップ: `~/.config/ghostty/config.bak-before-herdr-fork-20260904`
- fork ソース: このリポジトリ（ブランチ `herdr-sidebar-resources-agent-status`）
- Homebrew 版は削除していません。`brew` 管理下のまま残っています。

## 追加された機能

1. 左ペイン最下部に SSD 空き容量と Swap 使用量を固定表示（Agents 一覧のみスクロール）
   - SSD は起動時に即時取得し、以降 30 秒ごとに再取得。Swap は従来どおり 4 秒周期
   - 取得成功後は緑の `✓` を 5 秒間表示。値が変わらなくても、正常に再取得できれば表示する
   - `✓` 表示中に次の更新が成功した場合は、その時点から 5 秒を数え直す
   - 取得失敗時は最後の正常値を保持したまま黄色の `!` を表示し、次に成功するまで維持
   - 左ペインが狭いときは数値を優先し、マーカーから先に省略する
   - SSD 値は `statfs` の `f_bavail`（`df` 相当）。Finder の Purgeable 込み表示とは一致しないことがある
2. Agents 一覧に状態表示 `idle / run / wait / done / err`
   - 最長4文字の短縮表記。行の幅はエージェント名と作業時間に使いたく、状態は
     読むより見分けられれば足りるため。色とアイコンが同じ区別を担うので短くしても
     判別性は落ちない
   - `run` は処理中、`wait` はユーザーの入力・指示待ち、`err` は異常終了または
     要対応
   - 表示ラベルのみの変更で、状態機械・内部状態名・検出器の名前・socket API は
     従来どおり（`idle` / `working` / `blocked` / `done` / `unknown`）。
     `state_labels` の上書きキーも長い名前のまま
   - 検出上の `blocked` は「何を待っているか」に合わせて `wait` と表示
   - `run` はアクセントブルー `#89B4FA`（UI へ馴染む色）、`wait` はモーブ
     `#CBA6F7`（正常な指示待ち）、`err` は赤 `#FF5C89`（異常）。wait と err を
     色でも識別できる
   - `idle` `#82EC79` / `done` `#3DFFDF` は従来どおり、パレット固有の色相を保ったまま
     暗背景で読める彩度・明度へ補正
   - 状態文字と左の状態ドットは同じ対応表を参照するので色がずれない
3. Agent 行の状態表示の右に、実際に作業していた時間を表示
   - `claude · run 01:24` の形。進捗率でもプログレスバーでもなく**実測時間**
   - `run` 中のみ加算 / `wait` は人の応答待ちなので停止（表示は残る）
   - `done` `err` は最終累積時間で固定 / `idle` は非表示で、次のターンは 0 から
   - 1時間未満は `MM:SS`、1時間以上は `H:MM:SS`。秒未満は切り捨て、24時間で折り返さない
   - 色は状態色ではなくサイドバーの控えめな二次色。太字にしない
   - 幅が足りないときは時間だけを丸ごと省略（`01:` のように切らない）。
     エージェント名・状態文字は従来どおり
   - 項目は従来どおり2行。再描画は表示が変わる秒だけ
   - 作業せずに `wait` へ入ったターンは時間を表示しない（前ターンの値は持ち越さない）
   - 幅判定は実際に描く短いラベルで行うため、以前より狭い幅でも時間が残る
     （`run` なら 22 桁、`working` 時代は 26 桁が下限だった）
   - 計測は状態遷移から行うため、設定の再読み込み・サイドバーの折りたたみ・
     選択の切り替え・同一状態の重複報告ではリセットされない
   - 計測はサーバープロセス内のみ。サーバー再起動および live handoff では 0 に戻る
   - 専用スレッドは増やさず既存の deadline ループで更新。設定キーの追加もなし
4. Agents 一覧のタブ名を暖色アクセント `#FFB375` で表示
   - 固定文字列ではなく `tab` トークン単位の指定なので、任意のタブ名に適用される
     （自動命名タブや今後追加するタブも対象）
   - 選択中・非選択・Navigate モードのいずれでも維持される
   - ワークスペース名・区切り `·`・エージェント名・状態表示は従来色のまま

```text
● ~ · my-project
  claude · run 01:24

────────────────
SSD   188 GB ✓
SWAP  0.0 GB
```

作業時間の確認:

1. エージェントが動き出すと `00:00` から1秒ごとに増える
2. 質問待ちになると増加が止まり、答えると止まった値から再開する
3. 完了すると最終時間で固定される
4. `idle` では時間が消える。次に動き出すとまた `00:00` から
5. サイドバーを狭めると時間だけが消え、`claude · run` は残る
6. サーバーを再起動すると、稼働中のエージェントも `00:00` から数え直しになる

SSD 行のマーカーは次の 3 状態です。

```text
SSD   188 GB ✓   取得成功から 5 秒間
SSD   188 GB !   直近の取得に失敗（値は最後の正常値）
SSD   188 GB     通常表示
```

## 起動確認

新しい Ghostty ウィンドウで以下を確認してください。

```bash
herdr --version                     # PATH 次第では Homebrew 版が出ます
~/.local/bin/herdr --version
```

左ペイン最下部に SSD / SWAP が出ていれば切り替え成功です。
タブ名が `#FFB375` のオレンジになっていれば、サイドバーの配色も新しいバイナリで動いています。

SSD の定期更新は次の順で確認できます。

1. 起動直後に SSD 容量と緑の `✓` が出る
2. 約 5 秒後に `✓` だけが消え、数値と色はそのまま残る
3. 約 30 秒後にふたたび `✓` が 5 秒間出る
4. ゴミ箱を空にすると、遅くとも次の 30 秒周期で数値が動く
5. サイドバー幅を狭めると `✓` から先に消え、数値は残る

実機の答え合わせ:

```bash
df -h /System/Volumes/Data          # f_bavail 相当。フッターの SSD 値はこれと一致する
diskutil info /System/Volumes/Data | grep -i 'free space'
sysctl vm.swapusage
```

Finder やシステム設定の空き容量は解放可能な Purgeable 領域を加算するため、
`df` およびフッターの値より大きく出ます。一致しなくても不具合ではありません。

## バイナリを入れ替えたあとの反映

Herdr はサーバー／クライアント構成で、**サイドバーを描画しているのはサーバープロセス**です。
Ghostty を再起動してもクライアントが作り直されるだけで、既存のサーバーにアタッチし直すため、
バイナリを差し替えただけでは表示が変わりません。**バイナリを更新したら毎回、Herdr サーバーの
再起動が必要です。** サーバーを明示的に止めてください。

Herdr のペイン内で作業している場合、`server stop` は自分のセッションごと閉じます。
エージェントに自動実行させず、手元で実行してください。

```bash
~/.local/bin/herdr server stop
# そのあと Ghostty を手動で起動する
```

注意点:

- `herdr` と素で打つと PATH 上の別のインストール（この環境では Homebrew 版 0.8.0）に
  解決されることがあります。**必ず更新した実体のパスを指定**してください。
  この環境での実体は `~/.local/bin/herdr`（Ghostty の `initial-command` も同じ絶対パス）です。
- 反映されているか疑わしいときは、稼働中サーバーが実行している実体を確認します。
  `rm` してから `cp` する安全手順の性質上、古いプロセスは削除済み inode を保持し続けます。

```bash
pgrep -fl 'herdr server'
lsof -p <server-pid> | grep -w txt   # 表示された inode が現在のファイルと一致するか
```

## ロールバック

問題があれば元に戻せます。

```bash
cp ~/.config/ghostty/config.bak-before-herdr-fork-20260904 ~/.config/ghostty/config
~/.local/bin/herdr server stop
# Ghostty を再起動
```

これだけで Homebrew 版 0.8.0 に戻ります。

## footer を消したいとき

`~/.config/herdr/config.toml` に追記（Herdr 本体の再起動が必要）:

```toml
[ui.sidebar.resources]
enabled = false
```

## 再ビルド手順

```bash
export PATH="$(brew --prefix zig@0.15)/bin:$HOME/.cargo/bin:$PATH"
cd path/to/herdr
cargo build --release --locked
# 実行中のバイナリを上書きするとコード署名が無効化され、以後 SIGKILL される。
# 必ず削除してから新しい inode でコピーすること。
rm -f ~/.local/bin/herdr
cp target/release/herdr ~/.local/bin/herdr
~/.local/bin/herdr --version   # 動作確認
```

必要な toolchain: Rust 1.96.1（`rustup` 導入済み）、Zig 0.15.2（`brew install zig@0.15` 済み）。

## 併用している Claude Code 側の配色

Herdr のペイン内で動かす Claude Code には、このフォークの左ペイン配色と合わせて
次のテーマ上書きを使っています。Herdr 本体の設定ではなく、Claude Code 側の設定です
（この環境では `~/.claude/themes/herdr-dark.json`、`settings.json` の `theme` は
`custom:herdr-dark`）。

| キー | 値 | 反映先 |
|---|---|---|
| `userMessageBackground` | `#1A4A58` | ユーザー入力の背景 |
| `userMessageBackgroundHover` | `#215C6D` | 同じくホバー時 |
| `claude` | `#50FA7B` | アシスタント発話先頭の `⏺` とスピナー |
| `briefLabelClaude` | `#7EDB9A` | brief 表示の `Claude` ラベル |

Ghostty は `background-opacity = 0.6` / `background-opacity-cells = true` で動かしているため、
セル背景は背後と合成されて表示されます。ターミナルのセルに個別の alpha は無いので、
透け具合の調整は RGB 値そのもので行っています。

## 既知の制約

- `done` は非フォーカス時のみ表示されます（Herdr 既存仕様。フォーカス中の pane は即「既読」になり `idle`）。
- `err` はエージェントのプロセスが処理中に終了した場合のみです。同 pane で新しいエージェントが起動すると解除されます。
- footer は展開時のデスクトップサイドバー専用。折りたたみ時・mobile レイアウト時は非表示になり、取得処理も止まります。
- SSD の取得は既存の deadline ループ上で同期実行されます（`statfs` は常駐カーネルカウンタへの直接 syscall でマイクロ秒オーダー）。ゴミ箱を空にしても即座には反映されず、最大 30 秒遅れます。FSEvents 等の監視は行っていません。
- 起動後に一度も取得できていない場合は、マーカーだけを出さずに行ごと非表示になります。
- Windows 非対応。Linux は実装済みですが未検証です。
- `brew upgrade herdr` では更新されません。上流の新版に追従するには rebase して再ビルドしてください。

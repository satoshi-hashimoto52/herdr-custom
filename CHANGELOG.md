...
### Added
- The expanded sidebar now pins free disk space and swap use below the Agents list, so the Agents list scrolls while the readings stay visible. Values come from OS APIs rather than shelled-out commands — `statfs` on the APFS Data volume and `vm.swapusage` on macOS, `statvfs` and `/proc/meminfo` on Linux — and report the space a process can actually claim: `f_bavail`, the same figure `df` reports, which need not match the purgeable-inclusive number Finder and System Settings show. Free space is read at startup and every 30 seconds after that, while swap keeps its own four-second beat; both sample only while the footer is on screen, and mark yellow, peach, and red as space runs low or swap grows. Turn the footer off with `ui.sidebar.resources.enabled = false`.
- The sidebar's free-space row now says when it was last re-read. A green `✓` sits beside the reading for five seconds after a successful refresh — including a refresh that returns the same number — and a second success inside that window restarts the five seconds. A failed read keeps the last good value rather than blanking the row and shows a yellow `!` until a read succeeds, at which point the `!` becomes a fresh `✓`. Repeated failures log once rather than once per attempt, and a sidebar too narrow for both drops the marker to keep the number intact.
- Agent rows now print how long the agent has actually spent working, next to the state word: `claude · run 01:24`. The clock runs only while an agent is `run`, pauses while it is `wait`ing on a person, and freezes at the final total for `done` and `err`; `idle` shows nothing and the next turn starts again from zero. It is measured time rather than an estimate of progress, so there is no percentage and no bar. Under an hour it reads `MM:SS`, at or over one `H:MM:SS`, with part-seconds dropped rather than rounded up. The figure is the first thing a narrow row gives up — it is dropped whole rather than cut, and never at the cost of the agent name or the state word.
...
- Agents whose process exits mid-turn now read as `err`, based on the recorded process exit rather than pane output, and `error` is accepted as a `state_labels` key for per-pane overrides. The override key keeps the long name; only the drawn word is short.

### Changed
- Sidebar agent states now use short labels, at most four characters: `idle`, `run` for an agent that is processing, `wait` for one that is waiting on your input or instruction, `done`, and `err` for a process that exited mid-turn or otherwise needs attention. A row spends its width on the agent name and the working time, and the state only has to be recognised — colour and marker carry the same distinction, so the short forms lose nothing. `wait` is the detected `blocked` state named for what you are waiting on. This is the displayed label only — the state machine, the internal state names, the detector's own names, and the socket API keep their existing `idle`/`working`/`blocked`/`done`/`unknown` values, and `state_labels` overrides are still keyed by the long names.
- Sidebar `run` now takes the theme's accent blue and `wait` its mauve, both as the palette ships them. A lifted yellow sat too close to the warm tab-name accent to be found at a glance, and the waiting state shared the red with the error state; the two are now told apart by color as well as by the word, and red belongs to `err` alone. `idle`, `done`, and `unknown` are unchanged — they still hold their palette hue and move only in saturation and lightness, lifted far enough to read against a dark background. Light palettes are left as they are, and a palette built from ANSI color names, or one that ships the same color for both tokens, keeps the earlier yellow and red for these two states. The state word and the marker beside it read from one table, so they cannot drift apart. Display color only: the state machine, the internal state names, and the socket API are unchanged.
